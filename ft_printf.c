/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_printf.c                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tjerdnap <tjerdnap@student.42bangkok.co    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/01 09:45:46 by tjerdnap          #+#    #+#             */
/*   Updated: 2024/03/03 10:30:32 by tjerdnap         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

int	ft_print_char(char c, int i)
{
	write(1, &c, 1);
	i++;
	return (i);
}

int	ft_print_str(char *s, int i)
{
	if (!s)
		i = ft_print_str("(null)", i);
	else
	{
		while (*s)
		{
			write(1, &(*s), 1);
			s++;
			i++;
		}
	}
	return (i);
}

int	ft_print_int(int num, int i)
{
	if (num == -2147483648)
		i = ft_print_str("-2147483648", i);
	else if (num < 0)
	{
		i = ft_print_char('-', i);
		i = ft_print_int(-num, i);
	}
	else if (num > 9)
	{
		i = ft_print_int(num / 10, i);
		i = ft_print_char(num % 10 + '0', i);
	}
	else
	{
		i = ft_print_char(num + '0', i);
	}
	return (i);
}

int	ft_print_hex(unsigned int hex, int i, int lower)
{
	//unsigned int	x;

	//x = (unsigned int)hex;
	if (hex == 0)
		i = ft_print_char('0', i);
	else if (hex >= 16)
	{
		i = ft_print_hex(hex / 16, i, lower);
		i = ft_print_hex(hex % 16, i, lower);
	}
	else
	{
		if (hex <= 9)
			i = ft_print_char(hex + '0', i);
		else
		{
			if (lower)
				i = ft_print_char(hex + 'a' - 10, i);
			else
				i = ft_print_char(hex + 'A' - 10, i);
		}
	}
	return (i);
}

int	ft_find_format(const char *format, va_list ap, int i, int j)
{
	if (format[j] == 'c')
		i = ft_print_char(va_arg(ap, int), i);
	else if (format[j] == 's')
		i = ft_print_str(va_arg(ap, char *), i);
	else if (format[j] == 'd' || format[j] == 'i')
		i = ft_print_int(va_arg(ap, int), i);
	else if (format[j] == 'x')
		i = ft_print_hex(va_arg(ap, unsigned int), i, 1);
	else if (format[j] == 'X')
		i = ft_print_hex(va_arg(ap, unsigned int), i, 0);
	return (i);
}

int	ft_printf(const char *format, ...)
{
	va_list	ap;
	int		i;
	int		j;

	i = 0;
	j = 0;
	va_start(ap, format);
	while (format[j])
	{
		if (format[j] == '%')
			i += ft_find_format(format, ap, i, ++j);
		else
		{
			i++;
			write(1, &format[j], 1);
		}
		j++;
	}
	va_end(ap);
	return (i);
}

int	main(void)
{
	char c = 'n';
	//char s[10] = "Hello";
	char *s;
	char *p = NULL;
	// int d = 12;
	// int i = -345;
	int d = 123456780;
	int i = -2147483648;
	unsigned int u = 1234;
	int x = 12334;
	int X = 12334;

	//printf("\t  %%c \t%%s \t%%p \t\t%%d \t\t%%i \t%%u \t%%x \t%%X\n");
	//printf("printf:    %c\t%s\t %p\t %d\t %i\t %u\t %x\t %X\n", c, s, p, d, i,
	//	u, x, X);
	int me_c = ft_printf("%c", c); 
	int std_c = printf("%c", c);
	int me_s = ft_printf("%s", s); 
	int std_s = printf("%s", s);
	// int me_p = ft_printf("%p", p); 
	// int std_p = printf("%p", p);
	int me_d = ft_printf("%d", d); 
	int std_d = printf("%d", d);
	int me_i = ft_printf("%i", i); 
	int std_i = printf("%i", i);
	// int me_u = ft_printf("%u", u); 
	// int std_u = printf("%u", u);
	int me_x = ft_printf("%x", x); 
	int std_x = printf("%x", x);
	int me_X = ft_printf("%X", X); 
	int std_X = printf("%X", X);

	// COMPARISON OF RETURN VALUE	
	printf("\n");
	printf("me_c:  %d\n", me_c);
	printf("std_c: %d\n", std_c);
	printf("me_s:  %d\n", me_s);
	printf("std_s: %d\n", std_s);
	// printf("%d\n", me_p);
	// printf("%d\n", std_p);
	printf("me_d:  %d\n", me_d);
	printf("std_d: %d\n", std_d);
	printf("me_i:  %d\n", me_i);
	printf("std_i: %d\n", std_i);
	// printf("%d\n", me_u);
	// printf("%d\n", std_u);
	printf("me_x:  %d\n", me_x);
	printf("std_x: %d\n", std_x);
	printf("me_X:  %d\n", me_X);
	printf("std_X: %d\n", std_X);

	// ft_printf("ft_printf: %c\t %s\t %p\t %d\t %i\t %u\t %x\t %X\n", c, s, p, d, i, u, x, X);

		return (0);
}